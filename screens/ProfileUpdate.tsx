...
const ProfileUpdate = () => {
  ...
  const [maxJobPerSlot, setMaxJobPerSlot] = useState();
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      console.log('sfhsjfhjkh=>>>>', userInfo);
      ...
      setMaxJobPerSlot(userInfo.max_job_per_slot);
  ...
  const UpdateAccount = () => {
    setLoading(true);
    let data = {
      gas_safe_id_card: gsCard,
      oil_registration_number: oilRegistrationNumber,
      installer_ref_no: installerRefNo,
      max_job_per_slot: maxJobPerSlot,
    };
    UserUpdate(data).then(async res => {
      if (res) {
        console.log('Success:', res);
  ...
  const inputChangedAccountHandler = (inputId, inputValue) => {
    switch (inputId) {
      case 'gsCard':
        setGsCard(inputValue);
        break;
      case 'oilRegistrationNumber':
        setOilRegistrationNumber(inputValue);
        break;
      case 'installerRefNo':
        setInstallerRefNo(inputValue);
        break;
      case 'maxJobPerSlot':
        setMaxJobPerSlot(inputValue);
        break;
      default:
        break;
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      {titleData == 'Personal Information' ? (
        <>
      ...
      {titleData == 'Account Details' ? (
        <>
          ...
          <View style={styles.card}>
            ...
            <Input
              id="gsCard"
              onInputChanged={inputChangedAccountHandler}
              errorText={undefined} // No validation error
              placeholder="Gas Safe ID Card"
              placeholderTextColor={dark ? COLORS.grayTie : COLORS.black}
              value={gsCard}
            />
            ...
            <Input
              id="oilRegistrationNumber"
              onInputChanged={inputChangedAccountHandler}
              errorText={undefined} // No validation error
              placeholder="Oil Registration Number"
              placeholderTextColor={dark ? COLORS.grayTie : COLORS.black}
            ...
            <Input
              ...
              onInputChanged={inputChangedAccountHandler}
              errorText={undefined} // No validation error
              placeholder="Installer Ref Number"
              placeholderTextColor={dark ? COLORS.grayTie : COLORS.black}
              value={installerRefNo}
            />

            <Text style={styles.label}>Max Job Per Slot</Text>
            <Input
              id="maxJobPerSlot"
              onInputChanged={inputChangedAccountHandler}
              errorText={undefined} // No validation error
              placeholder="Max Job Per Slot"
              placeholderTextColor={dark ? COLORS.grayTie : COLORS.black}
              value={maxJobPerSlot.toString()}
            />
            <View style={styles.buttonRow}>
              <TouchableOpacity style={styles.cancelBtn} onPress={handleCancel}>
                <Text style={styles.cancelText}>Cancel</Text>
