...
const UpdateOccupiedAddress = ({route}) => {
  ...
  useEffect(() => {
    if (address.occupant_phone) {
      let {iso2, localNumber} = parseFullNumber(address.occupant_phone);
      setCountryCode(iso2);
      setPhoneNo(localNumber);
    }
    if (address.occupant_name) {
  ...
  const onUpdateAddress = async values => {
    ...
    const data = values;
    try {
      const res = await CustomerJobAddressUpdate(address.id, data);
      console.log('res', res);
      ...
      if (res && res.success) {
        ...
        const errorMessage = res.message || 'Server responded with an error.';
        Alert.alert('Update Failed', errorMessage);
        console.error('API Response Error:', res);
      }
    } catch (error) {
      console.error('Update failed due to network or execution error:', error);
      Alert.alert(
        'Error',
        'Could not update address. Please check your connection and try again.',
      );
    } finally {
      // setLoading(false);
    }
            ...
            <View style={styles.formContainer}>
              <Formik
                initialValues={{
                  ...
                  // Occupant fields
                  occupant_name: address?.occupant_name || '',
                  // occupant_phone: formattedPhoneNo || '',
                  occupant_email: address?.occupant_email || '',
                  note: address?.note || '',
                }}
                onSubmit={values => {
...
export default UpdateOccupiedAddress;
