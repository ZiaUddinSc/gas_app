...
export const CustomerUpdate = async (customerId, data) => {
  ...
  try {
    const response = await axios.put(
      `${Settings.baseUrl}${Settings.endpoints.customer_update(customerId)}`,
      data,
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const CustomerJobAddressUpdate = async (id, data) => {
  const token = await AsyncStorage.getItem('userToken');
...
export const CustomerJobAddressStore = async (customerId, data) => {
  const token = await AsyncStorage.getItem('userToken');

  try {
    const response = await axios.post(
      `${Settings.baseUrl}${Settings.endpoints.customer_job_address_store(
        customerId,
      )}`,
      data,
      {
